...
from .app import NationalWaterLevelForecastCanada as app
...
def home(request):
    ...
    return render(request, 'national_water_level_forecast_canada/home.html', context)
...
def get_popup_response(request):
    ...
    try:
        ...
        '''Get Observed Data'''
        df = pd.read_csv(os.path.join(app.get_app_workspace().path, 'Water_Level_Data/{}.csv'.format(codEstacion)), index_col=0)
...
def get_hydrographs(request):
    ...
    try:
        ...
        if (observed_df.index[-1] >= pd.to_datetime('1979-01-01')):
            observed_df = observed_df.loc[observed_df.index >= pd.to_datetime('1979-01-01')]
        ...
        return render(request, 'national_water_level_forecast_canada/gizmo_ajax.html', context)
...
def get_time_series_bc(request):
    ...
    try:
        ...
        url_rt = 'https://dd.weather.gc.ca/hydrometric/csv/{0}/daily/{0}_{1}_daily_hydrometric.csv'.format(province, codEstacion)

        try:
            df = pd.read_csv(url_rt)
            df.drop(columns=[' ID', 'Grade', 'Symbol / Symbole', 'QA/QC', 'Grade.1', 'Symbol / Symbole.1', 'QA/QC.1'], inplace=True)
            df.drop(columns=["Discharge / Débit (cms)"], inplace=True)
            df.rename(columns={"Water Level / Niveau d'eau (m)": "Water Level (m)"}, inplace=True)
        ...
        hydroviewer_figure.add_trace(template('Return Periods', (r2 - 0.05, r2 - 0.05, r2 - 0.05, r2 - 0.05), 'rgba(0,0,0,0)', fill='none'))
        ...
        hydroviewer_figure.add_trace(template(f'100 Year: {r100}', (r100, r100, max(r100 + 0.35, max_visible), max(r100 + 0.35, max_visible)), colors['100 Year']))
